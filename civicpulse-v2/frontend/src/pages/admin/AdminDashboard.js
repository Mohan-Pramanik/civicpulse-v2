...
const KPI_CONFIG = [
  { key:'total',      label:'Total Reports', icon:'📊', color:'#6366f1', glow:'rgba(99,102,241,0.3)', filter:'' },
...
function TrendChart({ trend }) {
  if (!trend?.length) return <div style={{ textAlign:'center', padding:'2rem', color:'var(--text-muted)', fontSize:13 }}>No trend data yet</div>;
  const maxVal = Math.max(...trend.flatMap(t => [t.reported || 0, t.resolved || 0]), 1);
  return (
    <div>
        ...
        {trend.map((t, i) => (
          <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
            <div style={{ width:'100%', display:'flex', gap:2, alignItems:'flex-end', height:100 }}>
              <div style={{ flex:1, height:`${Math.max(((t.reported||0)/maxVal)*100, (t.reported||0)>0?6:0)}%`, minHeight:(t.reported||0)>0?4:0, background:'linear-gradient(to top,#6366f1,rgba(99,102,241,0.4))', borderRadius:'4px 4px 0 0', boxShadow:'0 0 8px rgba(99,102,241,0.4)', position:'relative', transition:'height 0.8s ease' }}>
                {(t.reported||0) > 0 && <div style={{ position:'absolute', top:-16, left:'50%', transform:'translateX(-50%)', fontSize:9, color:'#818cf8', fontWeight:700 }}>{t.reported}</div>}
              </div>
              <div style={{ flex:1, height:`${Math.max(((t.resolved||0)/maxVal)*100, (t.resolved||0)>0?6:0)}%`, minHeight:(t.resolved||0)>0?4:0, background:'linear-gradient(to top,#22c55e,rgba(34,197,94,0.4))', borderRadius:'4px 4px 0 0', boxShadow:'0 0 8px rgba(34,197,94,0.4)', position:'relative', transition:'height 0.8s ease' }}>
                {(t.resolved||0) > 0 && <div style={{ position:'absolute', top:-16, left:'50%', transform:'translateX(-50%)', fontSize:9, color:'#4ade80', fontWeight:700 }}>{t.resolved}</div>}
              </div>
            </div>
            <div style={{ fontSize:9, color:'var(--text-muted)', marginTop:4 }}>{t._id?.slice(5)}</div>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{ padding:'12px 0', borderBottom:'1px solid var(--border)' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
        <div style={{ fontSize:13, fontWeight:600, color:'var(--text-primary)', fontFamily:'var(--f-display)' }}>{dept._id || 'Unknown'}</div>
        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
          <span style={{ fontSize:11, color:'var(--text-muted)' }}>{dept.total} total</span>
          <span style={{ fontSize:12, fontWeight:700, color, fontFamily:'var(--f-display)' }}>{rate}%</span>
        </div>
      </div>
      <div style={{ display:'flex', gap:4, marginBottom:6 }}>
        {[{l:'Resolved',v:dept.resolved,c:'#22c55e'},{l:'Progress',v:dept.inProgress,c:'#f59e0b'},{l:'Pending',v:dept.pending,c:'#ef4444'}].map(s => (
          <div key={s.l} style={{ flex:1, textAlign:'center', background:`${s.c}12`, border:`1px solid ${s.c}25`, borderRadius:6, padding:'4px 2px' }}>
            <div style={{ fontSize:13, fontWeight:700, color:s.c, fontFamily:'var(--f-display)' }}>{s.v||0}</div>
            <div style={{ fontSize:9, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.04em' }}>{s.l}</div>
          </div>
        ))}
      </div>
      <div className="progress"><div className="progress-bar" style={{ width:`${rate}%`, background:`linear-gradient(90deg,${color}88,${color})` }} /></div>
      {dept.avgDays && <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:4 }}>avg {Number(dept.avgDays).toFixed(1)} days to resolve</div>}
    </div>
  );
}
  ...
  return (
    <div className="page">
      <div className="page-header fade-up">
        <div><h1>Admin Dashboard</h1><p>KMC Control Centre · Real-time analytics</p></div>
      ...
      {/* Clickable KPI Cards → navigate to filtered issues */}
        ...
        {KPI_CONFIG.map((k, i) => (
          <div key={k.key} className="metric-card fade-up" style={{ animationDelay:`${i*0.08}s`, cursor:'pointer' }}
            ...
            <div className="metric-sub">
              {k.key==='resolved'   && `${kpis.resolutionRate}% rate`}
              {k.key==='inProgress' && `avg ${kpis.avgResolutionDays}d`}
              {k.key==='pending'    && `${kpis.critical} critical`}
              {k.key==='total'      && 'all time'}
            </div>
            <div style={{ fontSize:10, color:k.color, marginTop:6, opacity:0.7, fontFamily:'var(--f-display)', letterSpacing:'.05em' }}>CLICK TO FILTER →</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid-2" style={{ marginBottom:'1.25rem' }}>
        <div className="card fade-up d2">
          ...
          {(topAreas||[]).length === 0
            ? <div style={{ textAlign:'center', padding:'1rem', color:'var(--text-muted)', fontSize:13 }}>Add area when reporting issues to see hotspots</div>
            : (topAreas||[]).map((a,i) => (
        ...
        <div className="card fade-up d3">
          ...
          <div style={{ textAlign:'center', padding:'1.5rem 0' }}>
            {satisfaction?.avg > 0 ? (
              <>
                <div style={{ fontSize:36, filter:'drop-shadow(0 0 12px rgba(245,158,11,0.5))' }}>{'⭐'.repeat(Math.round(satisfaction.avg))}</div>
