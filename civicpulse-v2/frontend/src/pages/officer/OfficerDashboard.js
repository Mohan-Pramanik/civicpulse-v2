...
function IssueCard({ issue, onStatusChange }) {
  ...
  const changeStatus = async (status, message) => {
    setBusy(true);
    try {
      await updateStatus(issue._id, { status, message: message || note || '' });
  ...
  return (
    <>
      <div className="card" style={{ marginBottom:'1rem', borderLeft:`3px solid ${prio.color}`, boxShadow:`var(--s-sm), -2px 0 16px ${prio.glow}`, animation:'fadeUp 0.4s ease both' }}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {images.map((src, i) => (
                <div key={i} onClick={() => setPreviewIdx(i)}
                  style={{ width:80, height:62, borderRadius:8, overflow:'hidden', cursor:'pointer', border:'1px solid var(--glass-border)', position:'relative', flexShrink:0 }}
        ...
        {expanded && <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid var(--glass-border)', borderRadius:'var(--r-sm)', padding:'10px 14px', marginBottom:10, fontSize:13, color:'var(--text-secondary)', lineHeight:1.6 }}>{issue.description}</div>}
        {showNote && <div style={{ marginBottom:10 }}><textarea className="form-control" rows={2} placeholder="Add a note to citizen…" value={note} onChange={e=>setNote(e.target.value)} /></div>}

        <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
          {issue.status==='pending' && <button className="btn btn-sm" disabled={busy} onClick={() => changeStatus('assigned','Issue assigned to our team.')} style={{ background:'rgba(6,182,212,0.15)', color:'#06b6d4', border:'1px solid rgba(6,182,212,0.3)', borderRadius:'var(--r-sm)' }}>👷 Assign</button>}
          {['pending','assigned'].includes(issue.status) && <button className="btn btn-sm" disabled={busy} onClick={() => changeStatus('in_progress', note||'Work has started.')} style={{ background:'rgba(245,158,11,0.15)', color:'#f59e0b', border:'1px solid rgba(245,158,11,0.3)', borderRadius:'var(--r-sm)' }}>⚙️ In Progress</button>}
          {!['resolved','closed'].includes(issue.status) && <button className="btn btn-sm" disabled={busy} onClick={() => changeStatus('resolved', note||'Issue has been resolved. Thank you!')} style={{ background:'rgba(34,197,94,0.15)', color:'#22c55e', border:'1px solid rgba(34,197,94,0.3)', borderRadius:'var(--r-sm)', boxShadow:'0 0 10px rgba(34,197,94,0.15)' }}>✅ Resolve</button>}
          <button className="btn btn-glass btn-sm" onClick={() => setShowNote(n=>!n)}>📝 Note</button>
          {showNote && note && <button className="btn btn-sm" disabled={busy} onClick={() => changeStatus(issue.status, note)} style={{ background:'rgba(99,102,241,0.15)', color:'#818cf8', border:'1px solid rgba(99,102,241,0.3)', borderRadius:'var(--r-sm)' }}>Send</button>}
          <button className="btn btn-ghost btn-sm" style={{ marginLeft:'auto' }} onClick={() => setExpanded(e=>!e)}>{expanded?'▲ Less':'▼ More'}</button>
          <button className="btn btn-glass btn-sm" onClick={() => navigate(`/issues/${issue._id}`)}>View →</button>
        </div>
      </div>

      {previewIdx !== null && images.length > 0 && <ImagePreviewModal images={images} startIndex={previewIdx} onClose={() => setPreviewIdx(null)} />}
    </>
  );
}
...
export default function OfficerDashboard() {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    try {
      const [iRes, kRes] = await Promise.all([getAdminIssues({ ...filters, limit:50 }), getDeptStats()]);
  ...
  const KPI = [
    { key:'total', label:'Total Assigned', icon:'📋', color:'#6366f1', glow:'rgba(99,102,241,0.3)', filter:'' },
  ...
  return (
    <div className="page">
      <div className="page-header fade-up">
        <div><h1>Officer Dashboard</h1><p>🏛️ {user?.department} · {kpis?.total||0} issues</p></div>
      ...
      <div className="card fade-up d2" style={{ marginBottom:'1rem' }}>
        <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'flex-end' }}>
          ...
          {(filters.status||filters.priority||search) && <button className="btn btn-ghost btn-sm" onClick={()=>{setFilters({status:'',priority:''});setSearch('');}}>✕ Clear</button>}
        </div>
      </div>

      {busy ? [1,2,3].map(i=><SkeletonCard key={i}/>) : filtered.length===0
        ...
        : <div>{filtered.filter(i=>i.priority==='critical').map(i=><IssueCard key={i._id} issue={i} onStatusChange={onStatusChange}/>)}{filtered.filter(i=>i.priority!=='critical').map(i=><IssueCard key={i._id} issue={i} onStatusChange={onStatusChange}/>)}</div>
      }
    </div>
  );
}