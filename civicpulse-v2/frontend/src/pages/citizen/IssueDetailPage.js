  ...
  return (
    <div className="page page-narrow">
      ...
      <div className="card fade-up d1" style={{ marginBottom:'1rem' }}>
        <div style={{ fontSize:11, fontWeight:700, fontFamily:'var(--f-display)', background:'linear-gradient(135deg,#6366f1,#22c55e)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text', marginBottom:8 }}>{issue.ticketId}</div>
        ...
        <div style={{ marginTop:14, paddingTop:14, borderTop:'1px solid var(--border)', fontSize:12, color:'var(--text-muted)', display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:6 }}>
          <span>By <strong style={{ color:'var(--text-secondary)' }}>{issue.reportedBy?.name}</strong> · {new Date(issue.createdAt).toLocaleString()}</span>
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
            {images.map((src, i) => (
              <div key={i} onClick={() => setPreviewIdx(i)}
                ...
                <div style={{ display:'none', position:'absolute', inset:0, alignItems:'center', justifyContent:'center', background:'rgba(255,255,255,0.04)', flexDirection:'column', gap:6 }}>
                  <span style={{ fontSize:28 }}>🖼</span>
                  <span style={{ fontSize:10, color:'var(--text-muted)' }}>Unavailable</span>
                </div>
      ...
      {isOwner && issue.status==='resolved' && !issue.satisfactionRating && (
        <div className="card fade-up d3" style={{ marginBottom:'1rem', textAlign:'center' }}>
          ...
          <p style={{ fontSize:13, color:'var(--text-muted)', marginBottom:14 }}>How satisfied are you?</p>
