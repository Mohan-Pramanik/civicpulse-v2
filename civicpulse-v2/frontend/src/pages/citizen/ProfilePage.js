...
export default function ProfilePage() {
  ...
  const ROLE_LABEL = { admin:'Administrator', department: user?.isHead ? 'Department Head' : 'Field Officer', citizen:'Citizen' };

  return (
    <div className="page page-narrow">
      {/* Welcome Hero */}
      <div className="card fade-up" style={{ marginBottom:'1rem', background:'linear-gradient(135deg,rgba(99,102,241,0.1),rgba(34,197,94,0.05))', borderColor:'rgba(99,102,241,0.2)', padding:'1.5rem' }}>
        <div style={{ display:'flex', gap:18, alignItems:'center' }}>
          <div style={{ width:68, height:68, borderRadius:'50%', background:'linear-gradient(135deg,#6366f1,#22c55e)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--f-display)', fontSize:28, fontWeight:900, color:'#fff', flexShrink:0, boxShadow:'0 8px 24px rgba(99,102,241,0.4)' }}>
            {user?.name?.[0]?.toUpperCase()||'?'}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:12, color:'var(--text-muted)', fontFamily:'var(--f-display)', fontWeight:600, textTransform:'uppercase', letterSpacing:'.06em', marginBottom:4 }}>Welcome back,</div>
            <div style={{ fontFamily:'var(--f-display)', fontSize:22, fontWeight:900, color:'var(--text-primary)', letterSpacing:'-.3px' }}>{user?.name}</div>
            ...
            <div style={{ display:'flex', gap:8, alignItems:'center', marginTop:10, flexWrap:'wrap' }}>
              ...
              {user?.department && <span className="badge badge-blue">🏛️ {user.department}</span>}
