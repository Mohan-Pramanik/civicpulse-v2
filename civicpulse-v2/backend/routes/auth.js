...
// Creates citizen accounts (role defaults to 'citizen').
// Dept head/officer accounts are created via /api/admin/users
// or the seed route below.
// ─────────────────────────────────────────────────────────────
router.post('/register', registerRules, validate, asyncHandler(async (req, res) => {
  ...
  if (pincode && !/^[1-9][0-9]{5}$/.test(pincode)) {
    throw new ApiError('Invalid pincode. Must be a 6-digit Indian pincode (e.g. 700001).', 400);
  }
...
// Dev/setup helper — creates admin + 6 dept heads + 6 officers.
// ─────────────────────────────────────────────────────────────
const DEPARTMENTS = [
