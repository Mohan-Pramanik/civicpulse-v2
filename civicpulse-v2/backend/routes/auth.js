...
// GET /api/auth/seed-departments
// Creates admin + all department heads + all officers
router.get('/seed-departments', asyncHandler(async (req, res) => {
  ...
  for (const d of DEPARTMENTS) {
    ...
    await User.create({ name: d.head.name, email: d.head.email, password: d.head.password, role: 'department', department: d.dept });
    created.push({ role: 'department_head', dept: d.dept, email: d.head.email, password: d.head.password });

    // Officer (department role, same dept)
    await User.findOneAndDelete({ email: d.officer.email });
    await User.create({ name: d.officer.name, email: d.officer.email, password: d.officer.password, role: 'department', department: d.dept });
